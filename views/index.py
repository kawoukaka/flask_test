...
def register():
    if request.method == 'POST':
        if users.select().where(users.user_name == request.form['username']).count() < 1:
            ...
            users.create(user_name=request.form['username'], user_password=generate_password_hash(request.form['password']))
        ...
        # try:
            if users.select().where(users.user_name==request.form['username']).count() == 1:
                ...
                if check_password_hash(user.user_password,request.form['password']):
                    ...
                    user.update(user_status='online').where(user.user_id==session['id'])
